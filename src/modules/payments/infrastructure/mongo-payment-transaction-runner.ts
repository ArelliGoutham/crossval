...
export class MongoPaymentTransactionRunner implements PaymentTransactionRunner {
  ...
  async run<T>(operation: (tx: PaymentTransaction) => Promise<T>): Promise<T> {
    ...
    try {
      await session.withTransaction(async () => {
        const settlement = new MongoOrderSettlementPort(
          this.#database,
          session,
        );
        ...
        completion = {
          value: await operation({
            ...
            claimIdempotency: (record, now) => idempotency.claim(record, now),
            completeIdempotency: (
              merchantId,
              op,
              key,
